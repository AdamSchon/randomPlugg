...
///         ---- Explanation: ----
///
/// 1. mem _points_ to a linked list of internal pointers to allocated blocks (obj_t *).
...
///
/// Returns a pointer to the previous node's internal pointer to the block that contains p, or NULL if p is not found.
///
/// Example 1: if p is the pointer the start of Data42, the result below should be &x.
/// Example 2: if p is the pointer the start of Data52, the result below should be &(x->next->next).
///
///            ,--------------.      ,--------------.
///   x --->  H'H_Data42_MM   '---> H'H_Data47_MM   '--->H(null)H_Data52_MM
///   ^
///   '-- mem
///
///
...
bool kalloc_free(void *p)
{
  ...
  /// Note: first is a global variable, pointing to the first block in a list of kallocated blocks
...
bool kalloc_check_integrity(bool verbose)
{
  ...
  for (obj_t *cursor = first; cursor; cursor = cursor->next)
    {
