...
char *ioopm_strtok(char *src, char *delimiters, int *replacements)
{
  ...
  //Problem: Jag i++:ar av okänd anledning i denna loop. Det gör att jag hoppar över tecken i strängen.
  ...
  //Jag använder i++; även i denna loop av okänd anledning.
  for (; ; i++) {
    //NULL ska inte returneras ifall detta påstående är sant.
    //stored_src borde sättas till NULL och src returneras
    if (src[i] == '\0') {
      return(NULL);
    }
    if (is_delimiter(src[i], delimiters)){
